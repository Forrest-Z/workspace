// $Id: RBBM_radius.cpp 29890 2009-02-02 10:22:01Z tdelaet $
...
//ORO_CREATE_COMPONENT(BFL::RBBM_radius)

#define NUMCONDARGUMENTS_RBBM_radius 1
...
namespace BFL
{
  ...
  RBBM_radius::RBBM_radius( double p, double zmax, double sigma_m, Probability pi3, Probability pi4, ColumnVector scannerPos, string environment_file, int nbeams)
  ...
  {
#ifndef NDEBUG
    cout << "RBBM_radius constructor started " << endl;
#endif
    this->NumConditionalArgumentsSet(NUMCONDARGUMENTS_RBBM_radius);
    ConditionalPdf<vector<ColumnVector>,ColumnVector>::NumConditionalArgumentsSet(NUMCONDARGUMENTS_RBBM_radius);
    ...
    for(int i=0; i<_nbeams;i++) 
    {
        ...
        _beamModels[i]->NumConditionalArgumentsSet(NUMCONDARGUMENTS_RBBM_radius);
        ((ConditionalPdf<ColumnVector,ColumnVector>*)_beamModels[i])->NumConditionalArgumentsSet(NUMCONDARGUMENTS_RBBM_radius);
    }   
    ...
    if(!indata)  // file couldn't be opened
        cerr << "(RBBM_radius) Error: file " << _environment_file << " could not be opened" << endl;
    ...
    { 
     ...
     indata.close();
#ifndef NDEBUG
    cout << "RBBM_radius constructor finished " << endl;
#endif
  }

  /// Destructor
  RBBM_radius::~RBBM_radius()
  {
    //delete _phit, poccl, prand, pmax;
  }

  //Clone function
  RBBM_radius* RBBM_radius::Clone() const
  {
      return new RBBM_radius(*this);
  }

  void RBBM_radius::CalculateZstars() const
  {
#ifndef NDEBUG
    cout << "(RBBM_radius::CalculateZstars)  entered " << endl;
#endif
    ...
    _radiusState = this->ConditionalPdf<vector<ColumnVector>,ColumnVector>::ConditionalArgumentGet(0)(_state.size()); 
...
//                if( (distsq>pow(_radiusState,2.0)) && (D>=0) &&  ( ( (_relPos(1)*m)>=0) || (i==1 && _relPos(1)>=0) || (i==_nbeams && _relPos(1)<=0)  ) ) 
            ...
            case 2 :
                ...
                break;
            default:
                cerr << "(RBBM_radius::CalculateZstars) Error no solution found: case wrong implemented " << endl;
        }
...
//        cout<< "prob  measurement " << i << " " <<  (double)probOne / weight<< endl;
  ...
  bool
  RBBM_radius::SampleFrom (vector<Sample<vector<ColumnVector> > >& samples, const int num_samples, int method, void * args) const
  {
  ...
  bool
  RBBM_radius::SampleFrom (Sample<vector<ColumnVector> >& sample, int method, void * args) const
  {
        cout << "ERROR: not implemented yet" << endl;
        return false;
  }
  ...
  void
  RBBM_radius::ConditionalArgumentSet(unsigned int n_argument, const ColumnVector& argument)
  {
#ifndef NDEBUG
    cout << "(RBBM_radius::ConditionalArgumentSet)  entered " << endl;
#endif
...
#ifndef NDEBUG
    cout << "(RBBM_radius::ConditionalArgumentSet)  finished " << endl;
#endif
  }

  void
  RBBM_radius::ConditionalArgumentsSet(std::vector<ColumnVector> condargs)
  {
#ifndef NDEBUG
    cout << "(RBBM_radius::ConditionalArgumentsSet)  entered " << endl;
#endif
...
#ifndef NDEBUG
   cout << "(RBBM_radius::ConditionalArgumentsSet)  finished " << endl;
#endif
  }
} // End namespace
