// $Id: RBBMCartesian_radius.cpp 29890 2009-02-02 10:22:01Z tdelaet $
...
//ORO_CREATE_COMPONENT(BFL::RBBMCartesian_radius)

#define NUMCONDARGUMENTS_RBBMCARTESIAN_RADIUS 1
...
namespace BFL
{
  ...
  RBBMCartesian_radius::RBBMCartesian_radius( double p, double zmax, double sigma_m, Probability pi3, Probability pi4, ColumnVector scannerPos, string environment_file, int nbeams)
  ...
  {
#ifndef NDEBUG
    cout << "RBBMCartesian_radius constructor started " << endl;
#endif
    this->NumConditionalArgumentsSet(NUMCONDARGUMENTS_RBBMCARTESIAN_RADIUS);
    ConditionalPdf<ColumnVector,ColumnVector>::NumConditionalArgumentsSet(NUMCONDARGUMENTS_RBBMCARTESIAN_RADIUS);
    ...
    for(int i=0; i<_nbeams;i++) 
    {
        ...
        _beamModels[i]->NumConditionalArgumentsSet(NUMCONDARGUMENTS_RBBMCARTESIAN_RADIUS);
        ((ConditionalPdf<ColumnVector,ColumnVector>*)_beamModels[i])->NumConditionalArgumentsSet(NUMCONDARGUMENTS_RBBMCARTESIAN_RADIUS);
    }   
    ...
    if(!indata)  // file couldn't be opened
        cerr << "(RBBMCartesian_radius) Error: file " << _environment_file << " could not be opened" << endl;
    ...
    { 
     ...
     indata.close();
#ifndef NDEBUG
    cout << "RBBMCartesian_radius constructor finished " << endl;
#endif
  }

  /// Destructor
  RBBMCartesian_radius::~RBBMCartesian_radius()
  {
    //delete _phit, poccl, prand, pmax;
  }

  //Clone function
  RBBMCartesian_radius* RBBMCartesian_radius::Clone() const
  {
      return new RBBMCartesian_radius(*this);
  }

  void RBBMCartesian_radius::CalculateZstars() const
  {
#ifndef NDEBUG
    cout << "(RBBMCartesian_radius::CalculateZstars)  entered " << endl;
#endif
    ...
    _radiusState = this->ConditionalPdf<ColumnVector,ColumnVector>::ConditionalArgumentGet(0)(_state.size()); 
    // calculate relative position of state wrt scanner
    _relPos.resize(_state.sub(1,2).size());
    _relPos = _state.sub(1,2) - _scannerPos.sub(1,2);
...
//                if( (distsq>pow(_radiusState,2.0)) && (D>=0) &&  ( ( (_relPos(1)*m)>=0) || (i==1 && _relPos(1)>=0) || (i==_nbeams && _relPos(1)<=0)  ) ) 
            ...
            case 2 :
                ...
                break;
            default:
                cerr << "(RBBMCartesian_radius::CalculateZstars) Error no solution found: case wrong implemented " << endl;
        }
  ...
  Probability
  RBBMCartesian_radius::ProbabilityGet(const ColumnVector& meas) const
  {
#ifndef NDEBUG
    cout << "(RBBMCartesian_radius::ProbabilityGet)  entered " << endl;
#endif
    ...
    // measured distance
    _relPos = meas - _scannerPos.sub(1,2);
    meas1 = sqrt(pow(_relPos(1),2.0)+pow(_relPos(2),2.0));
    // middle beam
    double angle = atan2(_relPos(2),_relPos(1));
    beamMiddle = (int)(angle/ M_PI *(double)(_nbeams-1) ) ;
#ifndef NDEBUG
    cout << "(RBBMCartesian::ProbabilityGet)  distance measurement  " << meas1 << endl;
    cout << "(RBBMCartesian::ProbabilityGet)  angle " << angle << endl;
    cout << "(RBBMCartesian::ProbabilityGet)  beamMiddle " << beamMiddle << endl;
#endif
...
//    cout<< "prob  measurement " << i << " " <<  (double)probOne / weight<< endl;
...
#ifndef NDEBUG
    cout<< "prob " << prob << endl;
    cout << "(RBBMCartesian_radius::ProbabilityGet)  finished " << endl;
#endif
    return prob;
  }
  ...
  bool
  RBBMCartesian_radius::SampleFrom (vector<Sample<ColumnVector > >& samples, const int num_samples, int method, void * args) const
  {
  ...
  bool
  RBBMCartesian_radius::SampleFrom (Sample<ColumnVector> & sample, int method, void * args) const
  {
        cout << "ERROR: not implemented yet" << endl;
        return false;
  }
  ...
  void
  RBBMCartesian_radius::ConditionalArgumentSet(unsigned int n_argument, const ColumnVector& argument)
  {
#ifndef NDEBUG
    cout << "(RBBMCartesian_radius::ConditionalArgumentSet)  entered " << endl;
#endif
...
#ifndef NDEBUG
    cout << "(RBBMCartesian_radius::ConditionalArgumentSet)  finished " << endl;
#endif
  }

  void
  RBBMCartesian_radius::ConditionalArgumentsSet(std::vector<ColumnVector> condargs)
  {
#ifndef NDEBUG
    cout << "(RBBMCartesian_radius::ConditionalArgumentsSet)  entered " << endl;
#endif
...
#ifndef NDEBUG
   cout << "(RBBMCartesian_radius::ConditionalArgumentsSet)  finished " << endl;
#endif
  }
} // End namespace
