...
if plotPaper:
    ...
    for comp in range(1 , maxNumFilters+1):
        indicesCompLoca = nonzero(filterID == comp)[0]
        indicesCompLocb = nonzero(filterID == comp)[1]
        test = plot(estimateStateFiltersX[indicesCompLoca,indicesCompLocb] ,estimateStateFiltersY[indicesCompLoca,indicesCompLocb],marker = markersPlot[comp-1], color = colorsPlot[comp-1] , linestyle = 'None')
        if indicesCompLocb.size !=0:
    ...
    lgd = legend(legendEntries,legendText,numpoints=1,prop=props,loc='upper left')
    # set axis square
    axis("scaled")
    # set axis limits
    ...
    teller = 0;
    for cluster in range(0 , max(numClusters[:]) ):
        hold(True)
        # cluster positions
        test = plot(clusterX[:,cluster] ,clusterY[:,cluster],marker = 'd', color = 'k' , linestyle = 'None',label='component', markersize = 4)
        teller = teller + 1;
    legendEntries.append(test)
    legendText.append("cluster positions")
    # make legend 
    props = font_manager.FontProperties(size=14)
    lgd = legend(legendEntries,legendText,numpoints=1,prop=props,loc='upper left')
    # set axis square
    ...
    for comp in range(1 , maxNumFilters+1):
        ...
        if indicesComp.size != 0:
            if i>200:
                ...
                test = plot(estimateStateFiltersX[i-200:i,:][indicesCompLoca,indicesCompLocb] ,estimateStateFiltersY[i-200:i,:][indicesCompLoca,indicesCompLocb],marker = '.', color = colorsPlot[comp-1] , linestyle = 'None')
            else:
                indicesCompLoca = nonzero(filterID[0:i,:] == comp)[0]
                indicesCompLocb = nonzero(filterID[0:i,:] == comp)[1]
                test = plot(estimateStateFiltersX[0:i,:][indicesCompLoca,indicesCompLocb] ,estimateStateFiltersY[0:i,:][indicesCompLoca,indicesCompLocb],marker = '.', color = colorsPlot[comp-1] , linestyle = 'None')
            hold(True)
            teller = teller + 1;
