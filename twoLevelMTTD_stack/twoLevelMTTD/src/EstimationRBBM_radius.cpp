#include "EstimationRBBM_radius.hpp"

#include <ocl/ComponentLoader.hpp>

ORO_CREATE_COMPONENT( OCL::EstimationRBBM_radius)
...
namespace OCL
{
     EstimationRBBM_radius::EstimationRBBM_radius(std::string name)
         : TaskContext(name,PreOperational)
          ...
          ,_sysNoiseCovarianceValueRadius("sysNoiseCovarianceValueRadius","The covariance of the noise of the system model for the radius")
         ...
         , _onlyUseCentroidMeasurement("onlyUseCentroidMeasurements", "only use centroid measurements or not")
          ,_UpdateFilters("updateFilters", &EstimationRBBM_radius::updateFilters, this)
          ,_WriteTimings("writeTimings", &EstimationRBBM_radius::writeTimings, this)
          ...
          ,_features_unexplained(181)
     {
        ...
        assert( _sysNoiseCovarianceValueRadius.ready() );
        ...
        this->properties()->addProperty(&_sysNoiseCovarianceValueRadius);
    ...
    EstimationRBBM_radius::~EstimationRBBM_radius()
    {
    ...
    bool EstimationRBBM_radius::configureHook()
    {
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) ConfigureHook entered" << endlog();
#endif
        _dimension = 2 * (_level.value()+1) + 1 ;

#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) check properties " << endlog();
#endif
        // Check properties
        ...
        {
            log(Error) << "priorMean Property does not have correct dimension " << endlog();
            return false;
        }
        if( (_priorCovariance.value().size() != _dimension ) )
        {
            log(Error) << "priorCovariance Property does not have correct dimension " << endlog();
            return false;
        }
        if(_robotState.value().size() != _dimension )
        {
            log(Error) << "robotState Property does not have right dimension " << endlog();
            return false;
        }
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) checked properties " << endlog();
#endif
...
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) _priorMean " << _priorMean.value() << endlog();
#endif
...
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) prior covariance " << endlog();
#endif
        ...
        ************************/
        ...
        log(Debug) << "(EstimationRBBM_radius) dataAssociationFilter created " << endlog();
        ...
        ************************/
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) make system model " << endlog();
#endif
        vector<Matrix> AB(1);
        ...
        // noise for radius
        sysNoiseMatrix(_dimension,_dimension) = _sysNoiseCovarianceValueRadius.value();
        ...
        // system model for radius
        sysModelMatrix(_dimension,_dimension) = 1.0;
        ...
        log(Debug) << "(EstimationRBBM_radius) made system model " << endlog();
...
#endif
        _RBBM = new RBBMCartesian_radius(_p.value(),_zmax.value(),_sigma_m.value(),(Probability)(_pi3.value()),(Probability)(_pi4.value()),_robotState.value(),_environment_file.value(),_nbeams.value());
        ...
        ************************/
        ...
        if (!result)
        {
            log(Error) << "EstimationRBBM_radius does not have expected peers" << endlog();
...
#ifndef NDEBUG
        log(Debug) << "EstimationRBBM_radius has expected peers " << endlog();
#endif
        if (_prepareMeasurementNeeded)
    ...
    bool EstimationRBBM_radius::startHook()
    {
        log(Debug) << "(EstimationRBBM_radius) StartHook entered" << endlog();
        bool result = true;
        if (_prepareMeasurementNeeded)
        {
            prepareMeasurement();
            log(Debug) << "(EstimationRBBM_radius) Measurement prepared" << endlog();
        }
        calculatePriorVBClustersParameters();
        findClusters();
        log(Debug) << "(EstimationRBBM_radius) Clusters found" << endlog();
        estimateNumObjects();
        ...
        log(Debug) << "(EstimationRBBM_radius) number Clusters found" << _numClusters<< endlog();
        ...
        for (int meas=0 ; meas<_numMeasurements; meas++)
        {
            ...
            log(Debug) << "(EstimationRBBM_radius) measurement[" << meas << "]= " << measurements[meas]<< endlog();
...
*/
        ...
        updateFilters();
        log(Debug) << "(EstimationRBBM_radius) filters updated" << endlog();
        if (_reportingNeeded)
            snapshotReporting();
        log(Debug) << "(EstimationRBBM_radius) after snapshot" << endlog();
    ...
    void EstimationRBBM_radius::updateHook()
    {
#ifndef NDEBUG
        log(Debug) << "***************************************" << endlog();
        log(Debug) << "(EstimationRBBM_radius) UpdateHook entered" << endlog();
#endif
        if (_prepareMeasurementNeeded)
        {
            prepareMeasurement();
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) Measurements read " << endlog();
#endif
        }
...
#ifndef NDEBUG
        //log(Debug) << "(EstimationRBBM_radius) UpdateHook finished" << endlog();
#endif
#ifndef NDEBUG
        log(Debug) << "***************************************" << endlog();
#endif
    ...
    void EstimationRBBM_radius::stopHook()
    {
        this->writeTimings();
    }


    void EstimationRBBM_radius::cleanUpHook()
    {
    }

    bool EstimationRBBM_radius::updateFilters()
    {
#ifndef NDEBUG
        log(Debug) << "--------------------------" << endlog();
#endif
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) UpdateFilters entered" << endlog();
#endif
        // Get clusters of ports
        _stateClustersPort.Get(_clusters_loc);
        _numClustersPort.Get(_numClusters);
        ...
        {
            log(Debug) << "(EstimationRBBM_radius) teller_post " << teller_post << endlog();
            log(Debug) << "(EstimationRBBM_radius) filter expected value"<<  posts[teller_post]->ExpectedValueGet()<<endlog();
            log(Debug) << "(EstimationRBBM_radius) filter covariance"<<  posts[teller_post]->CovarianceGet()<<endlog();
        }
#endif
         // do system update
         _dataAssociationFilterMCPdf->Update(_sysModel);
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) System update done" << endlog();
        // print out data of posterior
...
#ifndef NDEBUG
           log(Debug) << "(EstimationRBBM_radius) find new objects " << endlog();
#endif
           this->findNewObjects(clusters);
#ifndef NDEBUG
           log(Debug) << "(EstimationRBBM_radius) new objects found" << endlog();
#endif
...
#ifndef NDEBUG
           log(Debug) << "(EstimationRBBM_radius) Set Variables for measurement update " << endlog();
           log(Debug) << "(EstimationRBBM_radius) _numClustersPort.Get() " << _numClustersPort.Get() << endlog();
#endif
...
#ifndef NDEBUG
           log(Debug) << "(EstimationRBBM_radius) Do Measurement update" << endlog();
#endif
           _dataAssociationFilterMCPdf->Update(_measModel,measurements);
#ifndef NDEBUG
           log(Debug) << "(EstimationRBBM_radius) Measurement update done" << endlog();
#endif
...
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) Old object deleted" << endlog();
#endif
    ...
    bool EstimationRBBM_radius::findNewObjects(vector<ColumnVector> clusters)
    {
    // add new filters for most unexplained clusters
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) findNewObjects entered" << endlog();
#endif
        ...
        {
#ifndef NDEBUG
            log(Debug) << "(EstimationRBBM_radius) num_filters: " << num_filters << endlog();
            log(Debug) << "(EstimationRBBM_radius) num_objects_estimated: " << num_objects_estimated << endlog();
            log(Debug) << "(EstimationRBBM_radius) num_filters==0 " << endlog();
#endif
            ...
            {
#ifndef NDEBUG
                log(Debug) << "(EstimationRBBM_radius) add filter for " << clusters[0] << endlog();
#endif
                // add new filter
                ...
                _priorCont.ExpectedValueSet(_priorMu);
#ifndef NDEBUG
                log(Debug) << "(EstimationRBBM_radius) prior expected value" << _priorCont.ExpectedValueGet() << endlog();
                log(Debug) << "(EstimationRBBM_radius) prior covariance" << _priorCont.CovarianceGet() << endlog();
#endif
...
#ifndef NDEBUG
            log(Debug) << "(EstimationRBBM_radius) new filter added" << _priorCont.CovarianceGet() << endlog();
#endif
        }
        ...
        {   
            ...
            log(Debug) << "(EstimationRBBM_radius) num_filters: " << num_filters << endlog();
            log(Debug) << "(EstimationRBBM_radius) num_objects_estimated: " << num_objects_estimated << endlog();
            log(Debug) << "(EstimationRBBM_radius) num_filters< num_objects_estimated " << endlog();
            log(Debug) << "(EstimationRBBM_radius) sort measurements according to most unexplained " << endlog();
#endif
            int num_features = clusters.size();
            log(Debug) << "(EstimationRBBM_radius) num_features: " << num_features << endlog();
...
#ifndef NDEBUG
            log(Debug) << "(EstimationRBBM_radius) measurements sorted " << endlog();
#endif
#ifndef NDEBUG
            log(Debug) << "(EstimationRBBM_radius) add filter for " << featuresWithProb[0].feature << endlog();
#endif
            // add new filter
            _priorMu = _priorMean.value();
...
#endif
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) findNewObjects finished" << endlog();
#endif
        return true;
    }

    bool EstimationRBBM_radius::deleteMostUncertainObject()
    {
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) deleteMostUncertainObject entered" << endlog();
#endif
        vector<Pdf<ColumnVector>* > posts = _dataAssociationFilterMCPdf->PostGet();
...
#ifndef NDEBUG
            log(Debug) << "(EstimationRBBM_radius) value_test "<< value_test  <<endlog();
#endif
...
#ifndef NDEBUG
                log(Debug) << "(EstimationRBBM_radius) filter to delete found with covariance"<<  posts[teller_post]->CovarianceGet()<<endlog();
#endif
            }
        }
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) remove filter "  << teller_mostUncertain<< endlog();
#endif
        _dataAssociationFilterMCPdf->RemoveFilter(teller_mostUncertain);
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) deleteMostUncertainObject done" << endlog();
#endif
        return true;
    }

    bool EstimationRBBM_radius::deleteOldObjects()
    {
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) deleteOldObjects entered" << endlog();
#endif
        vector<Pdf<ColumnVector>* > posts = _dataAssociationFilterMCPdf->PostGet();
        for(int teller_post = 0 ; teller_post < _dataAssociationFilterMCPdf->NumFiltersGet() ; teller_post++)
        {
...
#ifndef NDEBUG
        log(Debug) << "(EstimationRBBM_radius) deleteOldObjects done" << endlog();
#endif
        return true;
    }

    double EstimationRBBM_radius::calculateDistanceToRobot(ColumnVector targetState)
    {
        double distance = pow(targetState(1)-_robotState.value()(1) ,2.0) + pow(targetState(2)-_robotState.value()(2) ,2.0);
        return sqrt(distance);  
    }   

    bool EstimationRBBM_radius::calculatePriorVBClustersParameters()
    {
#ifndef NDEBUG
            log(Debug) << "(EstimationRBBM_radius::calculatePriorVBClustersParameters) entered " <<endlog();
#endif
        vector<Pdf<ColumnVector>* > posts = _dataAssociationFilterMCPdf->PostGet();
...
//#endif
    ...
    int EstimationRBBM_radius::factorial (int num)
    {
        if (num==1)
            return 1;
    ...
    bool EstimationRBBM_radius::writeTimings()
    {
        if(_reportingNeeded)
        {
