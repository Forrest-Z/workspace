 ...
 class CheckBackground{
   ...
   public:
	...
	void dataReceived2(const sensor_msgs::LaserScanConstPtr& laser_scan)
	{
	    ...
	    //TODO inser try-catch
	    if(high_fidelty_)
	      projector.transformLaserScanToPointCloud("map",*laser_scan, cloud,*tf_);
	    else
	    {       sensor_msgs::PointCloud tmp_cloud;
		    projector.projectLaser (*laser_scan, tmp_cloud);//, laser_max_range_, preservative_, mask);
		    tf_->transformPointCloud("map",  tmp_cloud, cloud);
	    }
	    
	    int tollerance = 8; //pixel, radius
	    bool out=false;
	    sensor_msgs::PointCloud clean_cloud;
	    sensor_msgs::PointCloud temp_cloud;
	    clean_cloud.header.stamp=ros::Time::now();
	    ...
	    for(unsigned i=0;i<cloud.points.size();i++)
	    {
	      	if(OutOfTheMap(cloud.points[i].x,cloud.points[i].y))
	      	{
	      	  out = true;	
	      	}
		else
	...
	bool OutOfTheMap(int x, int y)
	{
	  int gxa = (int) ((x - resp.map.info.origin.position.x)/resp.map.info.resolution);
	  int gya = (int) ((y - resp.map.info.origin.position.y)/resp.map.info.resolution);
	   if(resp.map.data[gxa + gya * resp.map.info.width]==-1)
		return true;
	  return false;
	}
