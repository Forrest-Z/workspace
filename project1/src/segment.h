...
uchar *originalImage;
Mat image2;
Mat image3=Mat::zeros(thresh2,imageWidth,CV_8UC3);
ObjectTrace *currentTrace;
ObjectTrace *startTrace;


void checkPixel(int x,int y,int pixelClass[imageHeight][imageWidth],int &xmax,int &xmin,int &ymax,int &ymin,int &zmax,int &zmin,int objectNo,int &j2)
{
...
xnew=x;ynew=y;
xnew=x+1;ynew=y;
if (xnew<imageWidth && pixelClass[ynew][xnew]==-2) 
    { 
     if ( fabs( originalImage[ynew*imageWidth+xnew]-D ) <= thresh1 )
	{
         checkPixel(xnew,ynew,pixelClass,xmax,xmin,ymax,ymin,zmax,zmin,objectNo,j2);
...
xnew=x;ynew=y;
xnew=x;ynew=y+1;
if (ynew<imageHeight && pixelClass[ynew][xnew]==-2) 
    { 
     if ( fabs( originalImage[ynew*imageWidth+xnew]-D ) <= thresh1 )
	{
         checkPixel(xnew,ynew,pixelClass,xmax,xmin,ymax,ymin,zmax,zmin,objectNo,j2);
