...
ros::Publisher position_vel_pub;
list<PeoplePositionVelocity*> people_position_velocity_;

nav_msgs::Path people_pos_vel_;
...
{
position_vel_pub_ = nh_.advertise<nav_msgs::Path>("people_position_vel",0);
...
for (it1=begin,int j=0;it1!=end;it1++,j++)
{
people_pos_vel_.pose[j].position.x=(*it1)->position_[0];
people_pos_vel_.pose[j].position.y=(*it1)->position_[1];
people_pos_vel_.pose[j].position.z=0;
people_pos_vel_.pose[j].orientation.x=(*it1)->v[0];
people_pos_vel_.pose[j].orientation.y=(*it1)->v1[0];
people_pos_vel_.pose[j].orientation.z=(*it1)->0;
}
}
